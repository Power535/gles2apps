...
#endif
{
    ...
    EGLint ai32ConfigAttribs[] = {EGL_BUFFER_SIZE, EGL_DONT_CARE,
    ...
    if (strstr(eglQueryString(display, EGL_VENDOR), "Broadcom")) {
        ...
        surface =
            eglCreateWindowSurface(display, config, gs_native_window, NULL);
    }
#endif
    ...
    usleep(50 * 1000);
    context =
        eglCreateContext(display, config, EGL_NO_CONTEXT, ai32ContextAttribs);
    if (!_eglTestError("eglCreateContext()"))
        return -1;

    printf("eglMakeCurrent()\n");
    usleep(50 * 1000);
    eglMakeCurrent(display, surface, surface, context);
    if (!_eglTestError("eglMakeCurrent()"))
        return -1;
